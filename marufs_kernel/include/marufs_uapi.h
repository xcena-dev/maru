...
#define MARUFS_PERM_ADMIN  0x0008 /* chown, perm_set_default (ownership control) */
...
/* Request structure for name-ref registration (name -> region:offset) */
struct marufs_name_offset_req
{
    char name[MARUFS_NAME_MAX + 1]; /* input: name */
    __u64 offset;                 /* input: offset within region's data area */
    __u64 name_hash;              /* input: pre-computed hash (0 = auto) */
};

/* Request structure for global name lookup */
struct marufs_find_name_req
{
    char name[MARUFS_NAME_MAX + 1];        /* input: name to search */
    char region_name[MARUFS_NAME_MAX + 1]; /* output: region file name (from RAT entry) */
    __u64 offset;                        /* output: offset within region's data area */
    __u64 name_hash;                     /* input: pre-computed hash (0 = auto) */
};

/* Batch find name entry (single lookup within batch) */
struct marufs_batch_find_entry
{
    char name[MARUFS_NAME_MAX + 1];        /* input: name to search */
    char region_name[MARUFS_NAME_MAX + 1]; /* output: region file name */
    __u64 offset;                        /* output: offset within region's data area */
    __u64 name_hash;                     /* input: pre-computed hash (0 = auto) */
    __s32 status;                        /* output: 0=found, negative errno */
    __u8 _pad[4];
};

/* Batch find name request (variable-length payload via pointer) */
struct marufs_batch_find_req
{
    ...
    __u64 entries; /* input/output: userspace pointer to marufs_batch_find_entry[] */
};

/* Batch name-offset store entry (single registration within batch) */
struct marufs_batch_name_offset_entry
{
    char name[MARUFS_NAME_MAX + 1]; /* input: name to register */
    __u64 offset;                 /* input: offset within region's data area */
    __u64 name_hash;              /* input: pre-computed hash (0 = auto) */
    __s32 status;                 /* output: 0=success, negative errno */
    __u8 _pad[4];
};

/* Batch name-offset store request (variable-length payload via pointer) */
struct marufs_batch_name_offset_req
{
    ...
    __u64 entries; /* input/output: userspace pointer to marufs_batch_name_offset_entry[] */
};
...
#define MARUFS_IOC_BATCH_NAME_OFFSET _IOWR('X', 6, struct marufs_batch_name_offset_req)
