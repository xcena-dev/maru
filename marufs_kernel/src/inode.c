                                        ...
                                        struct marufs_index_entry_cold* cold)
{
    ...
    inode->i_blocks = (inode->i_size + MARUFS_SECTOR_SIZE - 1) >> MARUFS_SECTOR_SHIFT;
    set_nlink(inode, 1);
}
                        ...
                        u32 shard_id, u32 entry_idx)
{
    ...
    if (!(inode->i_state & I_NEW))
    {
        ...
        {
            int rat_ret = marufs_resolve_rat_entry(sbi, xi, region_id);
            if (rat_ret == -ENOENT)
            {
                iput(inode);
 ...
 */
static int marufs_getattr(MARUFS_IDMAP_PARAM_COMMA const struct path* path, struct kstat* stat,
                        u32 request_mask, unsigned int query_flags)
{
    ...
    marufs_generic_fillattr(MARUFS_IDMAP_ARG_COMMA request_mask, inode, stat);
    return 0;
}
