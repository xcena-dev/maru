	...
	while (cur != MARUFS_BUCKET_END && cur < num_entries) {
		...
		if ((st == MARUFS_ENTRY_VALID_LE ||
			...
			if (strncmp(c->name, name, namelen) == 0 &&
				...
				if (st == MARUFS_ENTRY_INSERTING_LE) {
					...
					if ((created_at != 0 &&
						...
						if (cmpxchg(&e->state,
						    ...
						    MARUFS_ENTRY_INSERTING_LE) {
							pr_debug(
								"reclaimed stale INSERTING '%.*s' inline\n",
								(int)namelen,
								name);
				...
				pr_debug(
					"index_insert: name '%.*s' already exists\n",
					(int)namelen, name);
				return -EEXIST;
			}
		}
				 ...
				 u16 flags, u32 *out_entry_idx)
{
	...
	if (unlikely(!marufs_shard_geometry_valid(sc->num_buckets,
						  num_entries))) {
	...
	cold = NULL;
	{
		...
		if (sbi->shard_free_hint)
			hint = (u32)atomic_read(
				&sbi->shard_free_hint[shard_id]);
		if (hint >= num_entries)
			hint = 0;
	 ...
	 */
	{
		...
		while (cur != MARUFS_BUCKET_END && cur < num_entries) {
			...
			if (cur != entry_idx &&
				...
				struct marufs_index_entry_cold *c =
					&cold_entries[cur];
...
int marufs_index_delete_by_region(struct marufs_sb_info *sbi, u32 rat_entry_id)
{
	...
	/* Fast path: skip full scan if no name-refs registered for this region */
	{
		struct marufs_rat_entry *rat_e =
			marufs_rat_get(sbi, rat_entry_id);
	...
	for (s = 0; s < sbi->num_shards; s++) {
		...
		for (i = 0; i < num_entries; i++) {
			...
			if (READ_LE32(e->region_id) != rat_entry_id)
				continue;
			if (!(le16_to_cpu(READ_ONCE(e->flags)) &
