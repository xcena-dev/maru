...
#define MARUFS_REGION_IN_PROGRESS \
	0x00000001 /* Region allocation/release in progress */

/* ── RAT entry state ──────────────────────────────────────────────── */

#define MARUFS_RAT_ENTRY_FREE 0 /* Entry available */
#define MARUFS_RAT_ENTRY_ALLOCATING \
	1 /* Being initialized (fields not yet valid) */
#define MARUFS_RAT_ENTRY_ALLOCATED \
	2 /* Entry in use (fields fully initialized) */
...
#define MARUFS_REGION_HDR_POOL_SIZE \
	(MARUFS_MAX_REGIONS * MARUFS_REGION_HEADER_SIZE) /* 256 × 4KB = 1MB */
...
struct marufs_deleg_table {
	...
	struct marufs_deleg_entry
		entries[MARUFS_DELEG_MAX_ENTRIES]; /* 124 × 32B = 3968B */
...
static inline void __marufs_verify_structs(void)
{
	...
	MARUFS_BUILD_BUG_ON(sizeof(struct marufs_rat) !=
			    (4096 + 256 * MARUFS_MAX_RAT_ENTRIES));
}
...
static inline u32 marufs_bucket_idx(u64 name_hash, u32 num_buckets)
{
	if (unlikely(num_buckets == 0)) {
		pr_err(MARUFS_MODULE_NAME
		       ": BUG - num_buckets is 0, shard table corrupted!\n");
		return 0;
	}
