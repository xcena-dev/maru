                               ...
                               u64* out_offset)
{
    ...
    spans = kmalloc_array(MARUFS_MAX_RAT_ENTRIES, sizeof(*spans), GFP_KERNEL);
    if (!spans)
        return -ENOMEM;
    ...
    pr_debug("find_contiguous_space: size=%llu, regions_start=0x%llx, device_size=%llu\n",
                         ...
                         u64 size, u64 offset, u32* out_rat_entry_id)
{
    ...
    for (i = 0; i < MARUFS_MAX_RAT_ENTRIES; i++)
    {
        ...
        old_state = CAS_LE32(&entry->state,
        ...
        MARUFS_CXL_WMB(entry, sizeof(*entry)); /* Ensure all fields visible before state transition */
...
void marufs_rat_free_entry(struct marufs_sb_info* sbi, u32 rat_entry_id)
{
    ...
    MARUFS_CXL_WMB(entry, sizeof(*entry)); /* Ensure clears visible before state transition */

    /* CAS to FREE: try DELETING first, then ALLOCATED, then ALLOCATING (rollback) */
    cur_state = CAS_LE32(&entry->state, MARUFS_RAT_ENTRY_DELETING, MARUFS_RAT_ENTRY_FREE);
    if (cur_state != MARUFS_RAT_ENTRY_DELETING)
    {
        cur_state = CAS_LE32(&entry->state, MARUFS_RAT_ENTRY_ALLOCATED, MARUFS_RAT_ENTRY_FREE);
        if (cur_state != MARUFS_RAT_ENTRY_ALLOCATED)
            CAS_LE32(&entry->state, MARUFS_RAT_ENTRY_ALLOCATING, MARUFS_RAT_ENTRY_FREE);
    }
    MARUFS_CXL_WMB(entry, sizeof(*entry));
                     ...
                     u64 data_size)
{
    ...
    data_size_aligned = (data_size + MARUFS_ALIGN_2MB - 1) & ~(MARUFS_ALIGN_2MB - 1);
        ...
        while (retries < MARUFS_REGION_INIT_MAX_RETRIES)
        {
            ...
            else
            {
                ...
                if (waited > MARUFS_ALLOC_LOCK_TIMEOUT_NS)
                {
                    ...
                    wait_start = 0; /* reset for next attempt */
                    continue;
                }
            }
    ...
    MARUFS_CXL_RMB(sbi->rat, sizeof(*sbi->rat)); /* ensure lock acquisition visible before space scan */
