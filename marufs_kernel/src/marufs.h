...
struct marufs_shard_cache {
	...
	struct marufs_index_entry_hot
		*hot_entries; /* CXL hot entry array (32B each) */
	struct marufs_index_entry_cold
		*cold_entries; /* CXL cold entry array (96B each) */
	u32 num_buckets;
	u32 num_entries;
};
...
struct marufs_sb_info {
	...
	struct marufs_shard_header *
		shard_table; /* Shard header array (CXL, READ-ONLY after format) */
	...
	struct marufs_region_header
		*region_hdr_pool; /* 256 × 4KB in CXL memory */
...
marufs_rat_get(struct marufs_sb_info *sbi, u32 id)
{
					      ...
					      s32 delta)
{
	...
	do {
		...
		if (old_count == new_count)
			break;
	} while (CAS_LE32(&rat_e->name_ref_count, old_count, new_count) !=
		 old_count);
}

/* Safe region header accessor - returns header from consolidated pool by region_id */
static inline struct marufs_region_header *
marufs_region_hdr(struct marufs_sb_info *sbi, u32 region_id)
{
	if (unlikely(!sbi->region_hdr_pool ||
		     region_id >= MARUFS_MAX_RAT_ENTRIES))
		return NULL;
...
int marufs_index_insert(struct marufs_sb_info *sbi, const char *name,
			size_t namelen, u32 region_id, u64 file_size, u32 uid,
			u32 gid, u16 mode, u16 flags, u32 *out_entry_idx);
...
struct marufs_deleg_table *marufs_deleg_table_get(struct marufs_sb_info *sbi,
						  u32 rat_entry_id,
						  u32 *out_num_entries);
