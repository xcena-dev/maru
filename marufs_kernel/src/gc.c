...
static bool marufs_entry_reclaim_slot(struct marufs_index_entry_hot *e,
				      ...
				      struct marufs_sb_info *sbi, u32 shard_id,
				      u32 slot_idx)
{
	...
	if (cmpxchg(&e->state, expected_state, MARUFS_ENTRY_EMPTY_LE) ==
	    expected_state) {
...
bool marufs_gc_needs_sweep(struct marufs_sb_info *sbi, u32 shard_id)
{
	...
	return (tombstones * 100 / sbi->entries_per_shard) >=
	       MARUFS_GC_TOMBSTONE_THRESHOLD_PCT;
}
...
int marufs_gc_tombstone_sweep(struct marufs_sb_info *sbi, u32 shard_id)
{
	...
	for (i = 0; i < num_entries; i++) {
		...
		if (state == MARUFS_ENTRY_TOMBSTONE_LE) {
			...
			if (created_at == 0) {
				...
				MARUFS_CXL_WMB(&cold_entries[i],
					       sizeof(cold_entries[i]));
				continue; /* Re-check next GC cycle */
			}

			if ((now > created_at) &&
			    ((now - created_at) > MARUFS_STALE_TIMEOUT_NS)) {
				if (marufs_entry_reclaim_slot(
					    e, &cold_entries[i],
					    MARUFS_ENTRY_INSERTING_LE, sbi,
					    shard_id, i)) {
					inserting_reclaimed++;
					pr_debug(
						"gc reclaimed stale INSERTING entry shard %u slot %u (age %lluns)\n",
	...
	if (tombstone_reclaimed > 0) {
		marufs_le32_cas_dec(&sh->tombstone_entries,
				    (u32)tombstone_reclaimed);
		...
		if (READ_LE32(de->state) == MARUFS_DELEG_GRANTING) {
			...
			if (granted_at == 0 ||
				...
				if (CAS_LE32(&de->state, MARUFS_DELEG_GRANTING,
					...
					cleaned++;
					pr_debug(
						"gc reclaimed stale GRANTING deleg entry %u\n",
						i);
				}
			}
			continue;
