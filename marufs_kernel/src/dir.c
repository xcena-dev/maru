                       ...
                       struct dentry* dentry, umode_t mode, bool excl)
{
    ...
    if (!entry)
    {
        ...
        marufs_index_delete(sbi, dentry->d_name.name, dentry->d_name.len);
        marufs_rat_free_entry(sbi, rat_entry_id);
        return -EIO;
    }
                                       ...
                                       u32 rat_entry_id)
{
    ...
    old_state = CAS_LE32(&rat_e->state,
                         MARUFS_RAT_ENTRY_ALLOCATED, MARUFS_RAT_ENTRY_DELETING);
...
static int marufs_unlink(struct inode* dir, struct dentry* dentry)
{
    ...
    ret = marufs_check_permission(sbi, xi->rat_entry_id, MARUFS_PERM_DELETE);
    if (ret)
    {
...
static long marufs_ioctl_find_name(struct marufs_sb_info* sbi, unsigned long arg)
{
    ...
    /* Temporary region_id storage for deferred RAT reads */
    {
        ...
        for (i = 0; i < breq.count; i++)
        {
            ...
            r = marufs_index_lookup_hash(sbi, bent[i].name, nlen,
                                       bent[i].name_hash, &ie);
            if (r || !(le16_to_cpu(ie->flags) & MARUFS_ENTRY_FLAG_NAME_REF))
            {
        ...
        for (i = 0; i < breq.count; i++)
        {
            ...
            {
                struct marufs_rat_entry* rat_e = marufs_rat_get(sbi, rids[i]);
                if (rat_e)
                    prefetch(rat_e);
            }
        }
