...
static ssize_t marufs_read_iter(struct kiocb *iocb, struct iov_iter *to)
{
	...
	if (unlikely(xi->data_phys_offset == 0)) {
		pr_debug(
			"read on file with uninitialized region (ftruncate pending)\n");
...
static int marufs_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (sbi->dax_mode == MARUFS_DAX_HEAP && xi->data_phys_offset != 0) {
		...
		if (unlikely(!marufs_validate_slot_addr(
	...
	if (sbi->dax_mode == MARUFS_DAX_DEV && xi->data_phys_offset != 0) {
		...
		if (sbi->dax_filp && sbi->dax_filp->f_op &&
			...
			/* Translate file offset to device offset */
			{
				...
				vma->vm_pgoff += xi->data_phys_offset >>
						 PAGE_SHIFT;
				     ...
				     unsigned long arg)
{
	...
	{
		struct marufs_rat_entry *rat_e =
			marufs_rat_get(sbi, xi->rat_entry_id);
		u64 data_size;
	...
	ret = marufs_index_insert_hash(sbi, req.name, name_len, req.name_hash,
				       xi->rat_entry_id, req.offset,
				       current_uid().val, current_gid().val, 0,
				       MARUFS_ENTRY_FLAG_NAME_REF, &entry_idx);
	if (ret == -EEXIST) {
					   ...
					   unsigned long arg)
{
	...
	for (i = 0; i < breq.count; i++) {
		...
		r = marufs_index_insert_hash(sbi, bent[i].name, nlen,
					     ...
					     current_uid().val,
					     current_gid().val, 0,
					     MARUFS_ENTRY_FLAG_NAME_REF, &eidx);
		if (r == -EEXIST) {
			/* Upsert: update offset only if changed */
			r = marufs_index_lookup_hash(sbi, bent[i].name, nlen,
						     ...
						     &existing);
			if (r == 0 &&
			    (le16_to_cpu(existing->flags) &
			     MARUFS_ENTRY_FLAG_NAME_REF) &&
			    READ_LE32(existing->region_id) ==
				    xi->rat_entry_id) {
				if (READ_LE64(existing->file_size) !=
				    bent[i].offset) {
					WRITE_LE64(existing->file_size,
						   bent[i].offset);
					MARUFS_CXL_WMB(existing,
						       sizeof(*existing));
				}
				bent[i].status = 0;
				breq.stored++;
...
static long marufs_ioctl_clear_name(struct marufs_sb_info *sbi,
				    u32 caller_rat_entry_id, unsigned long arg)
{
	struct marufs_name_offset_req req;
	...
	if (!(le16_to_cpu(READ_ONCE(entry->flags)) &
	      MARUFS_ENTRY_FLAG_NAME_REF))
	...
	{
		struct marufs_rat_entry *rat_e =
			marufs_rat_get(sbi, caller_rat_entry_id);
		if (rat_e)
			marufs_rat_name_ref_adjust(rat_e, -1);
	}
...
static long marufs_ioctl_chown(struct marufs_sb_info *sbi,
			       struct marufs_inode_info *xi, unsigned long arg)
{
...
static long marufs_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	pr_debug("ioctl %s (0x%x) pid=%d rat=%u\n", marufs_ioctl_name(cmd), cmd,
		 current->pid, xi->rat_entry_id);
...
const struct file_operations marufs_file_ops = {
	...
	.get_unmapped_area =
		marufs_get_unmapped_area, /* 2MB alignment for DEV_DAX */
