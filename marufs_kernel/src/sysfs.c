    ...
    if (sbi->rat)
    {
        ...
        for (i = 0; i < MARUFS_MAX_RAT_ENTRIES; i++)
        {
            ...
            len += sysfs_emit_at(buf, len, "%u\t%u\t%u\t%s\t%llu\t0x%llx\t%s\n",
                                 i, owner_node, owner_pid, state_str,
                                 size, offset, name);
        }
    }
...
static ssize_t perm_info_show(struct kobject* kobj,
                              struct kobj_attribute* attr, char* buf)
{
    struct marufs_sb_info* sbi;
    int len = 0;
    ...
    for (i = 0; i < MARUFS_MAX_RAT_ENTRIES; i++)
    {
        struct marufs_rat_entry* entry = marufs_rat_get(sbi, i);
        u32 state, default_perms;
        ...
        default_perms = READ_LE32(entry->default_perms);

        {
            ...
            rhdr = marufs_region_hdr(sbi, i);
            if (rhdr)
            {
                dt = &rhdr->deleg_table;

                if (READ_LE32(dt->magic) == MARUFS_DELEG_MAGIC)
                    num_deleg = READ_LE32(dt->num_entries);

                len += sysfs_emit_at(buf, len, "%u\t0x%04x\t%u\n",
                                     i, default_perms, num_deleg);
            }
            else
            {
                len += sysfs_emit_at(buf, len, "%u\t0x%04x\t-\n",
                                     i, default_perms);
            }
        }
    }
                                ...
                                const char* buf, size_t count)
{
    ...
    reclaimed = marufs_gc_dead_process_regions(sbi);
    mutex_unlock(&marufs_sysfs_lock);
 ...
 */
static ssize_t gc_stop_store(struct kobject* kobj,
                             struct kobj_attribute* attr,
                             const char* buf, size_t count)
{
    mutex_lock(&marufs_sysfs_lock);
 ...
 */
static ssize_t gc_pause_show(struct kobject* kobj,
                             struct kobj_attribute* attr, char* buf)
{
    int len = 0;
    int i;
    ...
    {
        if (marufs_sysfs_sbi_list[i])
            len += sysfs_emit_at(buf, len, "node%u:%d ",
                                 marufs_sysfs_sbi_list[i]->node_id,
                                 atomic_read(&marufs_sysfs_sbi_list[i]->gc_paused));
    }
    mutex_unlock(&marufs_sysfs_lock);
...
static ssize_t gc_pause_store(struct kobject* kobj,
                              struct kobj_attribute* attr,
                              const char* buf, size_t count)
{
    u32 node_id;
    ...
    else
    {
        ...
        {
            if (marufs_sysfs_sbi_list[i])
                atomic_set(&marufs_sysfs_sbi_list[i]->gc_paused, pause ? 1 : 0);
        }
        mutex_unlock(&marufs_sysfs_lock);
