...
static int marufs_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	/* Aggregate active entries from all shards */
	{
		...
		for (i = 0; i < sbi->num_shards; i++)
			total_active +=
				le32_to_cpu(sbi->shard_table[i].active_entries);
...
static int marufs_format_device(struct marufs_sb_info *sbi)
{
	...
	total_hot_bytes =
		(u64)num_shards * entries_per_shard * MARUFS_HOT_ENTRY_SIZE;
	cold_entry_array_start = hot_entry_array_start + total_hot_bytes;
	total_cold_bytes =
		(u64)num_shards * entries_per_shard * MARUFS_COLD_ENTRY_SIZE;
	...
	regions_start =
		marufs_align_up(hdr_pool_offset + MARUFS_REGION_HDR_POOL_SIZE,
				MARUFS_ALIGN_2MB);
	...
	for (i = 0; i < num_shards; i++) {
		...
		sh->bucket_array_offset =
			cpu_to_le64(bucket_array_start +
				    (u64)i * buckets_per_shard * sizeof(u32));
		sh->hot_entry_array_offset = cpu_to_le64(
			hot_entry_array_start +
			(u64)i * entries_per_shard * MARUFS_HOT_ENTRY_SIZE);
		sh->cold_entry_array_offset = cpu_to_le64(
			cold_entry_array_start +
			(u64)i * entries_per_shard * MARUFS_COLD_ENTRY_SIZE);
	...
	/* --- Verify format: read back critical fields --- */
	{
		...
		pr_debug(
			"format verify shard[0]: magic=0x%x buckets=%u entries=%u\n",
		...
		vsh = (struct marufs_shard_header
				  ...
				  42ULL * MARUFS_SHARD_HEADER_SIZE);
		v_magic = le32_to_cpu(READ_ONCE(vsh->magic));
		v_buckets = le32_to_cpu(READ_ONCE(vsh->num_buckets));
		v_entries = le32_to_cpu(READ_ONCE(vsh->num_entries));
		pr_debug(
			"format verify shard[42]: magic=0x%x buckets=%u entries=%u\n",
			v_magic, v_buckets, v_entries);
	...
	pr_info("format complete (shards=%u, entries/shard=%u, rat@0x%llx, hdr_pool@0x%llx, regions@0x%llx)\n",
		num_shards, entries_per_shard, rat_offset, hdr_pool_offset,
		regions_start);
	return 0;
}
				  ...
				  struct super_block *sb, int silent)
{
	...
	MARUFS_CXL_RMB(
		gsb,
		sizeof(*gsb)); /* Memory barrier to see writes from other nodes */
...
static int marufs_init_shard_table(struct marufs_sb_info *sbi)
{
	...
	{
		u64 index_pool_offset =
			le64_to_cpu(sbi->gsb->index_pool_offset);
		...
		sbi->index_pool_base =
			(char *)sbi->dax_base + index_pool_offset;
	}

	/* Allocate per-shard free-slot hint in kernel RAM (local optimization) */
	sbi->shard_free_hint =
		kcalloc(sbi->num_shards, sizeof(atomic_t), GFP_KERNEL);
	...
	for (i = 0; i < sbi->num_shards; i++) {
		...
		if (!marufs_dax_range_valid(sbi, boff,
		    ...
		    !marufs_dax_range_valid(sbi, hot_off,
					    (u64)num_entries *
						    MARUFS_HOT_ENTRY_SIZE) ||
		    !marufs_dax_range_valid(sbi, cold_off,
					    (u64)num_entries *
						    MARUFS_COLD_ENTRY_SIZE)) {
		...
		sbi->shard_cache[i].hot_entries =
			(struct marufs_index_entry_hot *)((char *)sbi->dax_base +
							  hot_off);
		sbi->shard_cache[i].cold_entries =
			(struct marufs_index_entry_cold
				 *)((char *)sbi->dax_base + cold_off);
...
static int marufs_load_rat(struct marufs_sb_info *sbi)
{
	...
	/* Load region header pool (v2) */
	{
		...
		if (!marufs_dax_range_valid(sbi, pool_offset,
					    MARUFS_REGION_HDR_POOL_SIZE)) {
		...
		sbi->region_hdr_pool =
			(struct marufs_region_header *)((char *)sbi->dax_base +
							pool_offset);
		sbi->region_hdr_pool_offset = pool_offset;
		pr_debug(
			"region header pool at offset 0x%llx (%u × %u bytes)\n",
			pool_offset, MARUFS_MAX_RAT_ENTRIES,
			MARUFS_REGION_HEADER_SIZE);
	}

	pr_debug("RAT loaded at offset 0x%llx, RAT pointer=%p\n", rat_offset,
		 rat);
	pr_debug("  device_size=%llu, regions_start=0x%llx\n",
		 le64_to_cpu(rat->device_size), sbi->regions_start_offset);
