                                 ...
                                 unsigned long data_size_mb)
{
    ...
    for (int i = 0; i < num_names; i++)
    {
        ...
        ret = ioctl(fd, MARUFS_IOC_NAME_OFFSET, &req);
    ...
    if (num_names > 0)
    {
        ...
        ret = ioctl(fd, MARUFS_IOC_NAME_OFFSET, &req);
        TEST("upsert (same name, same offset) succeeds", ret == 0);
    }
    ...
    if (num_names > 1)
    {
        ...
        ret = ioctl(fd, MARUFS_IOC_NAME_OFFSET, &req);
        TEST("upsert (same name, different offset) succeeds", ret == 0);

        memset(&req, 0, sizeof(req));
        strncpy(req.name, names[0], sizeof(req.name) - 1);
        req.offset = offsets[0];
        ioctl(fd, MARUFS_IOC_NAME_OFFSET, &req);
    }
    ...
    for (int i = 0; i < num_names; i++)
    {
        ...
        ret = ioctl(fd, MARUFS_IOC_FIND_NAME, &freq);
    ...
    if (num_names >= 3)
    {
        ...
        ret = ioctl(fd, MARUFS_IOC_CLEAR_NAME, &req);
    ...
    if (num_names >= 3)
    {
        ...
        ioctl(fd, MARUFS_IOC_FIND_NAME, &f0);
        ioctl(fd, MARUFS_IOC_FIND_NAME, &f1);
    ...
    /* 8a: batch lookup of bulk entries registered in Test 6 */
    {
        ...
        ret = ioctl(fd, MARUFS_IOC_BATCH_FIND_NAME, &breq);
    ...
    {
        struct marufs_batch_find_entry bent[4];
        struct marufs_batch_find_req breq;
    ...
    /* 8f: latency comparison — N individual vs 1 batch */
    {
        ...
        for (int i = 0; i < bench_n; i++)
        {
            ...
            snprintf(freq.name, sizeof(freq.name),
                     "bulk-kv-entry-%d-%d", pid, i);
            ioctl(fd, MARUFS_IOC_FIND_NAME, &freq);
        }
        ...
        /* Batch FIND_NAME × 1 */
        {
            ...
            if (bent)
            {
                ...
                ioctl(fd, MARUFS_IOC_BATCH_FIND_NAME, &breq);
    ...
    /* 9a: batch store new entries */
    {
        ...
        ret = ioctl(fd, MARUFS_IOC_BATCH_NAME_OFFSET, &breq);
        ...
        {
            struct marufs_batch_find_entry* fbent;
            ...
            if (fbent)
            {
                ...
                ret = ioctl(fd, MARUFS_IOC_BATCH_FIND_NAME, &fbreq);
        ...
        /* 9c: batch upsert — same names, different offsets */
        for (int i = 0; i < batch_n; i++)
        {
        ...
        ret = ioctl(fd, MARUFS_IOC_BATCH_NAME_OFFSET, &breq);
        TEST("batch upsert (different offsets) succeeds", ret == 0);

        {
            char desc[128];
            snprintf(desc, sizeof(desc),
                     "batch upsert stored=%u/%d", breq.stored, batch_n);
            TEST(desc, (int)breq.stored == batch_n);
        }
        ...
        /* Verify updated offsets */
        {
            ...
            if (fbent)
            {
                ...
                ioctl(fd, MARUFS_IOC_BATCH_FIND_NAME, &fbreq);

                int match = 0;
                for (int i = 0; i < batch_n; i++)
                    if (fbent[i].status == 0 &&
                        fbent[i].offset == (__u64)(batch_n - 1 - i) * SLOT_SIZE)
                        match++;
                char desc[128];
                snprintf(desc, sizeof(desc),
                         "upsert offsets verified: %d/%d", match, batch_n);
                TEST(desc, match == batch_n);
        ...
        /* 9d: batch upsert — same names, same offsets (no-op) */
        for (int i = 0; i < batch_n; i++)
            bent[i].status = -1;
        ...
        ret = ioctl(fd, MARUFS_IOC_BATCH_NAME_OFFSET, &breq);
        TEST("batch upsert (same offsets, no-op) succeeds", ret == 0);
        TEST("no-op upsert stored count correct",
             (int)breq.stored == batch_n);
    ...
    /* 9h: latency comparison — individual vs batch store */
    {
        ...
        /* Batch NAME_OFFSET × 1 (upsert) */
        {
            ...
            if (bent)
            {
                ...
                ioctl(fd, MARUFS_IOC_BATCH_NAME_OFFSET, &breq);
                clock_gettime(CLOCK_MONOTONIC, &ts2);
