...
graph LR
    A[LMCache<br/>Client] -->|RPC<br/>ZMQ| B[MaruServer]
    B -->|ResourceManagerClient| RM[MaruResourceManager]
    RM -->|VFS| C[marufs<br/>kernel module]
    C --- D[CXL Memory]
```
...
|---------|----------|-------------|
| Region allocation | MaruShmClient → RM → `/dev/dax` | MarufsClient → RM → `open(O_CREAT)` + `ftruncate` on marufs mount |
| Region mapping | MaruShmClient.mmap() → fd from RM | MarufsClient.mmap() → fd from RM |
| Region deletion | MaruShmClient → RM → free extent | MarufsClient → RM → `unlink(region_file)` |
| Access control | **None** (`/dev/dax` wide open) | **Kernel VFS** — per-region permission via `perm_set_default`/`perm_grant` ioctl |
| Server process | MaruServer + Resource Manager | MaruServer + Resource Manager (RM handles marufs backend) |
...
Clients don't need any configuration change — DaxMapper automatically selects the appropriate backend based on server response. Both backends delegate allocation and permissions to the C++ Resource Manager via `ResourceManagerClient`.
...
| **Resource Manager** | Manages `/dev/dax` pools | Manages marufs region files + perm ioctls |
| **ResourceManagerClient** | Used by MaruShmClient | Used by MarufsClient |
...
sequenceDiagram
    ...
    AM->>XC: alloc(size)
    XC->>RM: ALLOC_REQ (UDS IPC)
    RM->>K: open(O_CREAT) + ftruncate(size)
    Note over K,CXL: Allocate contiguous CXL memory
    RM->>K: perm_set_default(PERM_ALL)
    RM-->>XC: MaruHandle + fd (SCM_RIGHTS)
    AM-->>S: MaruHandle
    ...
    H->>XC: mmap(handle) using cached fd
...
sequenceDiagram
    ...
    DM->>XC: mmap(handle)
    XC->>XC: get_fd(handle) via ResourceManagerClient → RM
    Note over XC: mmap using fd from RM
...
graph TB
    ...
    ORM --> DM
    DM --> XC
    XC -->|ResourceManagerClient| RM
    RPC -->|ZMQ| S
    S --> KV
    S --> AM
    AM -->|ResourceManagerClient| RM
    RM -->|VFS + ioctl| K
    K --- CXL
```
...
sequenceDiagram
    ...
    participant RM as ResourceManager (Node A)
    participant K_A as marufs kernel (Node A)
    participant CXL as CXL Shared Memory
    participant K_B as marufs kernel (Node B)
    participant RM_B as ResourceManager (Node B)
    participant C as Client (Node B)

    Note over S,CXL: Step 1 — Server requests region via RM (Node A)
    S->>RM: ALLOC_REQ (via ResourceManagerClient)
    RM->>K_A: open(O_CREAT) + ftruncate(size)
    K_A->>CXL: Allocate metadata + data area
    RM->>K_A: perm_set_default(PERM_ALL)
    RM-->>S: MaruHandle + fd

    Note over S,C: Step 2 — Client gets handle via RPC
    S-->>C: {handle, mount_path} (ZMQ response)

    Note over C,CXL: Step 3 — Client gets fd via RM (Node B)
    C->>RM_B: GET_FD_REQ (via ResourceManagerClient)
    RM_B->>K_B: open(region_file)
    K_B->>K_B: d_revalidate() → 0 (re-lookup from CXL)
    K_B->>CXL: marufs_lookup()
    RM_B-->>C: fd (SCM_RIGHTS)
    C->>C: mmap(fd, size)
