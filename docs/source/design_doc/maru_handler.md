...
**DaxMapper** manages the mmap/munmap lifecycle of CXL regions. It auto-selects between `MaruShmClient` (DAX mode) and `MarufsClient` (marufs mode) based on the server's `mount_path` response — this selection is transparent to all other components. Both backends delegate allocation and permissions to the C++ Resource Manager via `ResourceManagerClient`. DaxMapper creates mapped region objects that provide buffer views for direct memory access, and performs bulk unmap of all regions (both owned and shared) on `close()`.
