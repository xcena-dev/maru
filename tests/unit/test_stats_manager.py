...
class TestStatsManagerCounters:
    ...
    def test_record_increments_count(self):
        sm = StatsManager()
        sm.record("store", 1024, 50.0)
        sm.record("store", 2048, 30.0)
        stats = sm.get_stats()
    ...
    def test_hit_miss_merged(self):
        sm = StatsManager()
        sm.record("retrieve", 1024, 10.0, result="hit")
        sm.record("retrieve", 0, 5.0, result="miss")
        sm.record("retrieve", 2048, 15.0, result="hit")
        stats = sm.get_stats()
    ...
    def test_latency_stats(self):
        sm = StatsManager()
        sm.record("alloc", 100, 10.0)
        sm.record("alloc", 100, 30.0)
        sm.record("alloc", 100, 20.0)
    ...
    def test_multiple_op_types(self):
        sm = StatsManager()
        sm.record("alloc", 100, 10.0)
        sm.record("store", 200, 20.0)
        sm.record("retrieve", 300, 30.0, result="hit")
        stats = sm.get_stats()
    ...
    def test_per_client_stats(self):
        sm = StatsManager()
        sm.record("store", 100, 10.0, client_id="client_a")
        sm.record("store", 200, 20.0, client_id="client_b")
        sm.record("store", 300, 30.0, client_id="client_a")
        stats = sm.get_stats()
