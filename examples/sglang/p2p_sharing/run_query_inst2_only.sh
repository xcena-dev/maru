#!/bin/bash
# inst2에만 1회 쿼리 — inst1이 L3에 저장한 KV 캐시를 P2P로 히트하는지 확인용
...
echo "=== inst2 only — 1 query (P2P cache hit test) ==="
echo "Port: $PORT2"
echo ""

if ! curl -sf "http://localhost:${PORT2}/health" > /dev/null 2>&1; then
    echo "[ERROR] inst2 (port ${PORT2}) not reachable"
    exit 1
fi
...
echo "--- Check inst2.log for batch_exists hits and #cached-token ---"
echo "  grep 'batch_exists\|cached-token\|batch_get' inst2.log | tail -10"
