...
"""P2P KV cache sharing query test with TTFT measurement.

Replaces curl-based run_request.sh with async streaming for accurate TTFT timing.
Sends identical prompts to Instance 1 (store) then Instance 2 (retrieve),
measuring time-to-first-token to validate KV cache P2P transfer.
...
def print_box_summary(s1_results: list, s2_results: list, wait_time: float) -> None:
    ...
    print(f"{_BLUE}  P2P KV Cache Sharing - Results{_NC}", file=sys.stderr)
...
async def main():
    parser = argparse.ArgumentParser(
        description="P2P KV cache query test with TTFT measurement"
    )
    parser.add_argument("--model", default=DEFAULT_MODEL)
    parser.add_argument(
        "--port1",
        type=int,
        default=int(os.environ.get("LMCACHE_INST1_PORT", 8000)),
        help="Instance 1 port (default: $LMCACHE_INST1_PORT)",
    )
    parser.add_argument(
        "--port2",
        type=int,
        default=int(os.environ.get("LMCACHE_INST2_PORT", 8001)),
        help="Instance 2 port (default: $LMCACHE_INST2_PORT)",
    )
    ...
    print(
        f"\n[Session 1] Instance 1 (port {args.port1}) - Store KV cache",
        file=sys.stderr,
    )
    ...
    print(
        f"\n[Session 2] Instance 2 (port {args.port2}) - Retrieve KV cache",
        file=sys.stderr,
    )
    ...
    print_box_summary(s1_results, s2_results, args.wait_time)

    # Print machine-parseable JSON on stdout (last line)
